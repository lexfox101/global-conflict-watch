...
export function StatCard({ value, label, note }: StatCardProps) {
  return (
    <div className="flex h-full flex-col border-t border-rule pt-4">
      <p className="font-mono text-[30px] font-semibold leading-none tracking-[-0.03em] text-paper">{value}</p>
      <p className="mt-3 text-[13px] font-semibold text-paper-dim">{label}</p>
      <p className="mt-1.5 text-[12px] leading-relaxed text-paper-faint">{note}</p>
    </div>
  );
}
