      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-2 pb-2">
        {incidents.length === 0 ? (
          <div className="flex h-52 flex-col items-center justify-center px-6 text-center" role="status">
            ...
            <p className="mt-3 text-sm font-medium text-muted">No matching incidents</p>
            <p className="mt-1 text-xs leading-5 text-faint">Adjust or reset the filters to restore the demo feed.</p>
          </div>
            ...
            <article key={incident.id} className={`feed-row my-0.5 overflow-hidden transition ${selected ? "is-selected" : ""}`}>
              <button type="button" onClick={() => onSelect(incident.id)} aria-pressed={selected} className="w-full px-3 py-3 text-left">
                <div className="flex items-center gap-2">
                  ...
                  <time dateTime={incident.timestamp} className="ml-auto font-mono text-[10px] text-faint">{formatTime(incident.timestamp)}Z</time>
                </div>
                <h3 className={`headline-item mt-2.5 text-[15px] ${selected ? "text-ink" : "text-ink-body"}`}>{incident.title}</h3>
              ...
              {selected && (
                <div className="selected-summary mx-3 mb-3 pt-3">
                  <p className="text-[12px] leading-5 text-muted">{incident.summary}</p>
